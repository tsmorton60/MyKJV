...
async function displayBibChap(chap, vsList) {
    ...
    // Add chap to beginning 
    if (fromHistoryBack == false) {
        chapHistory.unshift(chap);
        vsHistory.unshift(vsList);
        if (chapHistory.length > 5) {
            // Remove oldest
            chapHistory.pop();
            vsHistory.pop();
        }
    ...
    console.log(chapHistory, 'vsHistory') 
...
function searchBib(term) {
	...
	if (term.indexOf('|') > -1) {
		...
		for (const [key, value] of Object.entries(searchText)) {
            ...
            // positive lookbehind doesn't work on Safari (Webkit) browers so trying this, (?:^|\\s|>)
	...
	var mkStr = allWords.map(word => "\\b" + word + "\\b").join('|');
...
function displayPopup(evt, title, body) {
 ...
 function onRefClick(evt) {
	...
	if (evt.target.nodeName.toLowerCase() == 'ref' ) {
        ...
        // To cause history to go back to the next verse
        histIdx = 1;
...
function histBack(evt) { 
    
    fromHistoryBack = true;
    console.log(chapHistory[histIdx], currentChap);
    
    if (chapHistory.length > 1) {
        displayBibChap(chapHistory[histIdx], vsHistory[histIdx]);
    
        histIdx +=1;
        if (histIdx > chapHistory.length -1) {
            histIdx = 0;
        }
    }
...
function parseQuery(pageUrl) {
	console.log(pageUrl) 
	
	showInitialChap();
	
	var query = {};
	var pairs = (pageUrl[0] === '?' ? pageUrl.substr(1) : pageUrl).split('&');
	for (var i = 0; i < pairs.length; i++) {
        var pair = pairs[i].split('=');
        query[decodeURIComponent(pair[0])] = decodeURIComponent(pair[1] || '');
	}

		if (query && 'list' in query) {
			
			var refStr = query['list']
			var refList = refStr.split(',');
			
            var page = ['<div style="padding-top: 10px; padding-bottom: 15px; text-align:center; background-color: #efefef;"><span style="color: brown; "><b>Reference List From Custom Link</b></span>\
						 <br><span style="font-size: smaller">Tap reference to view entire chapter.<br>Tap <span style="color: brown;">Topics</span> Tab below to return.</span></div>']
            
			for (let r of refList) {
				
				let r1 = r.slice(0, 3);
				if (isNaN(r1.charAt(0))) {
					r1 = r1.charAt(0).toUpperCase() + r1.charAt(1) + r1.charAt(2)
				} else {
					r1 = r1.charAt(0) + r1.charAt(1).toUpperCase() + r1.charAt(2)
				}
				...
				if (ref.search('-') != -1) {
					...
					for (var i = start; i <= end; i++) {
						var refLink = '<ref id="' + chap + i + '">' + chap + i + '</ref>';
						var vsTxt = avBib[chap + i].replace(/<fn>(\d+)<\/fn>/g, '')
						page.push('<div class="linkList"><b>' + refLink + '</b>' + ' ' + vsTxt + '</div>');
					}
					
					page.push('<hr style="width:80%; text-align:center; border-top: 1px solid lightgray;" />')
				
				} else {
					var refLink = '<ref id="' + ref + '">' + ref + '</ref>';
					var vsTxt = avBib[ref].replace(/<fn>(\d+)<\/fn>/g, '')
					//console.log(vsTxt) 
			...
			changeTab('topic-panel', document.getElementById('list-tab'));
			document.getElementById('topics').innerHTML = page.join('');
		} 
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('histIcon').addEventListener('click', histBack);
